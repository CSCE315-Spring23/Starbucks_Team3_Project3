...
function InventoryManage(){
    ...
    const handleDelivery = () => {
        fetch("http://localhost:5000/management/restock", {
            ...
            body: JSON.stringify({ "name": selectedItem.inventory_name, "amount": restockAmount })
        }).then(response => response.json())
    }
    const handleVoid = () => {
    ...
    const AddForm = () => {
        ...
        const handleSubmit = (e) => {
            ...
            fetch("http://localhost:5000/management/inventory", {
                ...
                body: JSON.stringify({ "name": name, "amount": amount, "cost": cost})
            }).then(response => response.json())
            fetch("http://localhost:5000/management/inventory")
                .then(response => response.json())
                .then(result => setMenuTable(result));
        ...
        <div>
            ...
            <div className='manager-action-bar'>
                <div className='managing-buttons'>
                    ...
                    <button className='manage-button' onClick={() => handleRemoveInvItemClick(selectedItem)}>Remove Inventory Item</button>
                    <button className='manage-button' onClick={() => handleModifyInvItemClick(selectedItem)}>Modify Inventory Item</button>
